...
- **Highly automated** Rarity, type, name, modifiers, and effects are all rolled and applied for you. The only things
...
Beyond strength, there are four modifier types. Each one stacks differently across items on your character sheet.
...
Just getting handed random magic items felt boring. Looting after every fight felt tedious and unfair. So instead, players earn special **envelopes** and **wishes** they can spend on gambling that's exciting, frictionless, and fair*.
...
- **Customization** Disable anything you don't want, nerf or buff to your heart's content
- **Author your own** Custom modifiers, vouchers, and wishes use the same format as the built-ins and drop straight
  into the pool
- **Standard Foundry items everywhere** Items, vouchers, and wishes are normal Foundry VTT items, with their values
  stored in flags
- **No overload** Modifiers are designed with GM overload in mind: the taxing work falls on the player, so you can
  keep on killing them!

## Try it yourself

Seen enough? Play with the actual interface, open pulls, inspect items, and explore modifiers, right in your browser:
...
> - It showcases the components in isolation, without Foundry VTT or a character sheet, so character-dependent flows and live game state won't behave like the real module
