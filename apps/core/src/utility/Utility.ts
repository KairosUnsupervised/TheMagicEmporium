...
export class Utility {
    ...
    private modifiersOverview = () => {
        ...
        const universal = (restriction: Restriction) => this.registry.weighted.filter((modifier) => {
            return modifier.application.restriction === restriction && modifier.application.whitelistedBy.length === 0;
        });
        return {
            all: byRestriction(),
            universal: {
    ...
    private tagsOverview = () => {
        return {
            equipment: this.countTags(Object.values(equipmentDetails).flatMap((item) => item.tags)),
            whitelist: this.countTags(this.registry.weighted.flatMap((modifier) => modifier.application.whitelistedBy)),
            blacklist: this.countTags(this.registry.weighted.flatMap((modifier) => modifier.application.blacklistedBy)),
            apply: this.countTags(this.registry.weighted.flatMap((modifier) => modifier.application.applies)),
        }
    }

    private countTags = (tags: string[]): { total: number, tags: [string, number][] } => {
    ...
    private getModifiers = (restriction: Restriction, tag?: Tag) => {
        return this.registry.weighted.filter((modifier) => {
            const matchesRestriction = modifier.application.restriction === restriction;
            if (!tag) return matchesRestriction;
            return matchesRestriction && modifier.application.whitelistedBy.includes(tag);
        })
    }
