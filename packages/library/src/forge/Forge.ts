...
export class Forge {
     ...
     */
    public static random = (equipment?: Equipment, rarity?: Rarity): ForgeProcess => {
    ...
    private static generateModifiers = (process: ForgeProcess, template: Template): void => {
    ...
    private static getRandomModifier = (process: ForgeProcess): Modifier => {
        ...
        Logger.error("Could not find a valid modifier after 10000 attempts", {process});
    ...
    private static generateEconomy = (process: ForgeProcess, template: Template): void => {
    ...
    private static generateName = (process: ForgeProcess): void => {
        ...
        const adjective = details.adjectives?.length
            ? details.adjectives[Math.floor(Math.random() * details.adjectives.length)]
            : '';
