import {ChangeOperation, ChangeSchema, validateChangeSchema} from "./change.schema";
...
export class Change {
     ...
     * @param changes
     */
    public static applyAll = <T extends object>(target: T, changes: Change[]): T => {
        ...
        for (const change of changes) {
            ...
            switch (operation) {
                ...
                case ChangeOperation.Append:
                    node[lastKey] = Array.isArray(current) ? [...current, value] : [value];
                    break;
