...
type CreateDefinition = FeatSchema &
	Required<Pick<FeatSchema, "title" | "description" | "disclaimer">>;
...
export class Feat {
	...
	public static createMultiple = (
		...
		return definitions.map((definition) => {
			...
			return Feat.create(
				{ ...defaultFlavor, ...(definition as FeatSchema) },
				icon,
			);
		});
	};
	...
	private static create = (definition: CreateDefinition, icon: Icon): Feat => {
		...
		if (definition.system) {
			...
			if (activities && activities.length > 0) {
				base.system = {
					...base.system,
					activities: Activity.activitiesToRecord(
						Activity.createMultiple(activities),
					),
				};
			}
		}
