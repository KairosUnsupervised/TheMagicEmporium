...
export const IndependentModifierDisplay = (props: IndependentModifierDisplayProps) => {
	...
	const items = breakpoints.map(bp => <IndependentBody modifier={props.modifier} data={{float: bp.min}}/>);

	return <BreakpointSwap items={items} defaultActiveIndex={activeIndex} type={ModifierType.Independent}/>;
};
